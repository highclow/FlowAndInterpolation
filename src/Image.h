...
void Image<T>::dx(Image<T1>& result,bool IsAdvancedFilter) const
{
    ...
    if(IsAdvancedFilter==false) {
        ...
        const T* pElement2 = this->data() + nChannels;
...
void Image<T>::desaturate(Image<T1> &image) const
{
    ...
    for(int i=0;i<nPixels;i++) {
        if(colorType == RGB) {
            data[i]=static_cast<double>(pData[offset]*.299+pData[offset+1]*.587+pData[offset+2]*.114);
        } else {
            data[i]=static_cast<double>(pData[offset]*.114+pData[offset+1]*.587+pData[offset+2]*.299);
        }
        offset += 3;
    }
}
